...
% a.Cl_B    Derivada do angulo de derrapagem da ASA com a Fuselagem
...
if p1 <= 1.5
    ...
    if p1 > 1.5 && p1 <= 3
      y = -2*10^-08*x^3 + 6*10^-07*x^2 - 7*10^-05*x + 2*10^-05;
        ...
        if p1 > 3 && p1 <= 5
      y = -2*10^-08*x^3 + 4*10^-07*x^2 - 7*10^-05*x + 2*10^-05;
           ...
           y = -3*10^-08*x^3 + 5*10^-07*x^2 - 8*10^-05*x - 0.0001;
              ...
              if p1 > 7
                y = -3*10^-08*x^3 + 5*10^-07*x^2 - 7*10^-05*x; %-0.0001;
...
p= a.A/cos(a.enf_c2_rad);
x = c.M*cos(a.enf_c2_rad);                    
...
x2 = (a.b/2)/cos(a.enf_c2_rad); %Comprimento da asa seguindo a linha de c2

b.lf = a.X_LE + (1/2)* a.c_barra + sin(a.enf_c2_rad)*x2; % Distancia do nariz a ponta de asa seguindo a linha de c2 
...
p = a.A/cos(a.enf_c2_rad);
 ...
 if p <= 4
     y = -0.0382*x^4 + 0.0856*x^3 - 0.058*x^2 + 0.0117*x + 0.9998;
...
if p2 < 0.25
    ...
    if p2>= 0.25 && p2<0.75
        y = 5*10^-06*x^4 - 7*10^-05*x^3 + 9*10^-05*x^2 + 0.0021*x - 0.008 + 4*10^-4;
    else
        if p2>= 0.75
...
if p4>0.75
    if p3<20
        ...
        if p3<20
        y = 1*10^-07*x^4 - 2*10^-06*x^3 + 1*10^-05*x^2 - 7*10^-05*x + 8*10^-16 +2*10^-4;
...
if p <= 3
     ...
     if p>3 && p <=5
         y = -0.651*x^4 + 1.4294*x^3 - 0.7274*x^2 + 0.1495*x + 0.9998;
...
DCl_B_diedro = -0.0005*sqrt(a.A)*(d/a.b)^2;

q8= DCl_B_diedro;

%% DCl_B_zw [deg^-1] depende de A,zw,b,d

DCl_B_zw = -(1.2*sqrt(a.A)/57.3) * (a.zw/a.b) * (2*d/a.b);
...
%Simulando Figura 7.17
%dado geométrico assumido do projeto
teta = 0; %torção

%% Calculo de Cl_B_WB [contribuição da asa para Cl)

Cl_B_WB1 = 57.3*(q1*(q2*q3*q4)+q5);
Cl_B_WB2 = a.diedro*(q6*q7+q8);
Cl_B_WB3 = q9+ teta*tan(a.enf_c4)*q10;
a.Cl_B = Cl_B_WB1+Cl_B_WB2+Cl_B_WB3;
